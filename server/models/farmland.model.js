...
const farmlandsSchema = mongoose.Schema(
  {
    ...
    farmlandType: {
      type: String,
      enum: {
        values: ["pomar novo", "pomar estabelecido", "Tipo de pomar não definido"],
        message: "{VALUE} nao e um tipo de pomar permitido",
      },
      default: function () {
        // get an array of all the sowing years for each division
        if (this.divisions && this.divisions.length){
          let sowingYears = this.divisions.map((division) => division.sowingYear);
    ...
    divisions: [
      {
        ...
        spacing: {
          ...
          category: {
            ...
            default: function () {
              if ((this.spacing.x && this.spacing.y) && ( this.spacing.x === this.spacing.y) ){
                return "regular";
              } else {
                return "irregular";
              }
